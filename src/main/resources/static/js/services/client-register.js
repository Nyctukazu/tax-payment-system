...
function registerNewBusiness() {
    ...
    attachNbGrossCurrencyInput(dialog);
    ...
    nbSubmitBtn.onclick = () => withSubmitLock(nbSubmitBtn, () => {
        ...
        const gross = parseCurrency(grossRaw);
...
function renderCreateApp() {
    ...
    // Attach currency inputs (idempotent via data flag)
    ['app-gross', 'app-exempt'].forEach(id => {
        const el = document.getElementById(id);
        if (el && !el.dataset.currencyBound) {
            el.dataset.currencyBound = '1';
            attachCurrencyInput(el, () => {
                updateCalc();
                // real-time validation
                const gross = parseCurrency(document.getElementById('app-gross')?.value) || 0;
                const exempt = parseCurrency(document.getElementById('app-exempt')?.value) || 0;
                const grossEl = document.getElementById('app-gross');
                const exemptEl = document.getElementById('app-exempt');
                const errGross = document.getElementById('err-app-gross');
                const errExempt = document.getElementById('err-app-exempt');
                if (grossEl) {
                    if (gross <= 0) { grossEl.classList.add('input-error'); errGross?.classList.remove('hidden'); }
                    else { grossEl.classList.remove('input-error'); errGross?.classList.add('hidden'); }
                }
                if (exemptEl) {
                    if (exempt > gross) { exemptEl.classList.add('input-error'); errExempt?.classList.remove('hidden'); }
                    else { exemptEl.classList.remove('input-error'); errExempt?.classList.add('hidden'); }
                }
            });
        }
    });
    ...
    if (descEl && !descEl.dataset.rtBound) {
        descEl.dataset.rtBound = '1';
        descEl.addEventListener('input', () => {
            const val = descEl.value;
            const errEl = document.getElementById('err-app-desc');
            if (!isNonEmptyTrimmed(val, 10)) {
                descEl.classList.add('input-error');
                if (errEl) { errEl.textContent = 'Please provide a detailed description (min 10 chars).'; errEl.classList.remove('hidden'); }
            } else if (containsProfanity(val)) {
                descEl.classList.add('input-error');
                if (errEl) { errEl.textContent = 'Description contains inappropriate language.'; errEl.classList.remove('hidden'); }
            } else {
...
function renderSettings() {
    ...
    if (langSel) langSel.value = s.language || 'English';
    renderSettingsToggles();
    attachCharCounter('set-name', 'counter-set-name', 50);

    const savedSize = DB.settings.fontSize || 'medium';
    applyFontSize(savedSize, false);

    [
        ['set-name',  'err-set-name',  v => v.length > 2],
        ['set-email', 'err-set-email', v => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)],
        ['set-phone', 'err-set-phone', v => v.length >= 7],
    ].forEach(([id, errId, validator]) => {
        const el = document.getElementById(id);
        if (el && !el.dataset.rtBound) {
            el.dataset.rtBound = '1';
            el.addEventListener('input', () => {
                const val = el.value.trim();
                const err = document.getElementById(errId);
                if (!validator(val)) { el.classList.add('input-error'); err?.classList.remove('hidden'); }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedFontSize = DB.settings.fontSize || 'medium';
...
>>>>>>> 4b1cc9b8a96262f3200be74229c819cc7856a58a
    }
...
/* =========================================================
   NEW) CURRENCY INPUT HELPERS
   ========================================================= */

/**
 * Parse a formatted currency string like "1,234,567.89" → 1234567.89
 */
function parseCurrency(str) {
    if (!str) return 0;
    // Remove all commas, trim spaces
...
/**
 * Format a raw number string into comma-separated with optional decimals.
 * Preserves cursor position as best as possible.
 * Rules:
 *  - Only digits, commas, and one decimal point allowed
 *  - Commas auto-inserted every 3 digits in integer part
 *  - If no decimal typed → on blur add .00; on input keep as is
 *  - User may type their own decimal
 */
function attachCurrencyInput(inputEl, onChange) {
    if (!inputEl) return;

    function formatValue(raw, forBlur) {
        ...
        if (forBlur) {
            // On blur: always show .00 or user's decimal rounded to 2
            const decFormatted = decPart !== undefined ? decPart.padEnd(2, '0').slice(0, 2) : '00';
            return intPart + '.' + decFormatted;
        } else {
            // During typing: show decimal only if user typed a dot
    ...
    inputEl.addEventListener('input', function (e) {
        ...
        const digitsBeforeCursor = oldVal.slice(0, selStart).replace(/[^\d.]/g, '').length;

        const formatted = formatValue(this.value, false);
        this.value = formatted;

        // Restore cursor: find position after same number of non-comma chars
        let count = 0;
        let newPos = 0;
    ...
    inputEl.addEventListener('blur', function () {
        ...
        if (!raw || raw === '0' || raw === '') {
            ...
            this.value = formatValue(raw, true);
        }
        if (onChange) onChange();
    });
    ...
    inputEl.addEventListener('paste', function (e) {
        ...
        this.value = formatValue(clean, false);
        if (onChange) onChange();
    });
}

/* =========================================================
   NEW) SETTINGS — EDIT PROFILE DIALOG
   ========================================================= */
function openEditProfile() {
    ...
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:10000;display:flex;align-items:center;justify-content:center;';
    const dialog = document.createElement('div');
    dialog.style.cssText = 'background:var(--bg-dark-cards);border:1px solid var(--border-color);border-radius:12px;padding:28px;max-width:480px;width:90%;color:var(--text-primary);box-shadow:0 20px 60px rgba(0,0,0,0.5);';
    dialog.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <h3 style="margin:0;font-size:1.05rem;"><i class="fa-solid fa-pen-to-square" style="color:var(--accent-blue);margin-right:8px;"></i>Edit Profile</h3>
            <button id="ep-close" style="background:none;border:none;color:var(--text-muted);font-size:1.2rem;cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div style="display:flex;flex-direction:column;gap:14px;">
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">Full Name</label>
                <input type="text" id="ep-name" class="form-control" maxlength="50" value="${s.name || ''}">
                <span class="error-text hidden" id="ep-err-name">Name must be at least 3 characters.</span>
            </div>
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">Email Address</label>
                <input type="email" id="ep-email" class="form-control" maxlength="100" value="${s.email || ''}">
                <span class="error-text hidden" id="ep-err-email">Please enter a valid email address.</span>
            </div>
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">Mobile Number</label>
                <input type="text" id="ep-phone" class="form-control" maxlength="15" value="${s.phone || ''}">
                <span class="error-text hidden" id="ep-err-phone">Please enter a valid contact number.</span>
            </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:20px;">
            <button id="ep-save" class="btn btn-green" style="flex:1;"><i class="fa-solid fa-check"></i> Save Profile</button>
            <button id="ep-cancel" class="btn btn-secondary" style="flex:1;">Cancel</button>
        </div>`;
    overlay.appendChild(dialog);
    document.body.appendChild(overlay);

    // Real-time validation
    const rtValidate = (id, errId, validator) => {
        const el = dialog.querySelector('#' + id);
        const err = dialog.querySelector('#' + errId);
        el.addEventListener('input', () => {
            if (!validator(el.value.trim())) { el.classList.add('input-error'); err.classList.remove('hidden'); }
            else { el.classList.remove('input-error'); err.classList.add('hidden'); }
        });
    };
    rtValidate('ep-name', 'ep-err-name', v => v.length > 2);
    rtValidate('ep-email', 'ep-err-email', v => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v));
    rtValidate('ep-phone', 'ep-err-phone', v => v.length >= 7);

    const closeDialog = () => { overlay.style.opacity='0'; overlay.style.transition='opacity 0.2s'; setTimeout(()=>overlay.remove(),200); };
    dialog.querySelector('#ep-close').onclick = closeDialog;
    dialog.querySelector('#ep-cancel').onclick = closeDialog;
    overlay.addEventListener('click', e => { if (e.target === overlay) closeDialog(); });

    dialog.querySelector('#ep-save').onclick = () => {
        const name = dialog.querySelector('#ep-name').value.trim();
        const email = dialog.querySelector('#ep-email').value.trim();
        const phone = dialog.querySelector('#ep-phone').value.trim();
        let ok = true;
        if (name.length < 3) { dialog.querySelector('#ep-name').classList.add('input-error'); dialog.querySelector('#ep-err-name').classList.remove('hidden'); ok = false; }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { dialog.querySelector('#ep-email').classList.add('input-error'); dialog.querySelector('#ep-err-email').classList.remove('hidden'); ok = false; }
        if (phone.length < 7) { dialog.querySelector('#ep-phone').classList.add('input-error'); dialog.querySelector('#ep-err-phone').classList.remove('hidden'); ok = false; }
        if (!ok) return;
        DB.settings.name = name; DB.settings.email = email; DB.settings.phone = phone;
        saveStore(DB);
        renderSettings();
        closeDialog();
...
/* =========================================================
   NEW) SETTINGS — CHANGE PASSWORD DIALOG
   ========================================================= */
function openChangePassword() {
    const overlay = document.createElement('div');
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:10000;display:flex;align-items:center;justify-content:center;';
    const dialog = document.createElement('div');
    dialog.style.cssText = 'background:var(--bg-dark-cards);border:1px solid var(--border-color);border-radius:12px;padding:28px;max-width:420px;width:90%;color:var(--text-primary);box-shadow:0 20px 60px rgba(0,0,0,0.5);';
    dialog.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <h3 style="margin:0;font-size:1.05rem;"><i class="fa-solid fa-lock" style="color:var(--accent-blue);margin-right:8px;"></i>Change Password</h3>
            <button id="cp-close" style="background:none;border:none;color:var(--text-muted);font-size:1.2rem;cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div style="display:flex;flex-direction:column;gap:14px;">
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">Current Password</label>
                <div style="position:relative;">
                    <input type="password" id="cp-current" class="form-control" placeholder="Enter current password" style="padding-right:42px;">
                    <button onclick="togglePwVis('cp-current',this)" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--text-muted);cursor:pointer;"><i class="fa-solid fa-eye"></i></button>
                </div>
                <span class="error-text hidden" id="cp-err-current">Current password is required.</span>
            </div>
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">New Password</label>
                <div style="position:relative;">
                    <input type="password" id="cp-new" class="form-control" placeholder="Min 8 characters" style="padding-right:42px;">
                    <button onclick="togglePwVis('cp-new',this)" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--text-muted);cursor:pointer;"><i class="fa-solid fa-eye"></i></button>
                </div>
                <div id="cp-strength-bar" style="height:4px;border-radius:2px;background:var(--border-color);margin-top:6px;overflow:hidden;"><div id="cp-strength-fill" style="height:100%;width:0%;transition:width 0.3s,background 0.3s;"></div></div>
                <span id="cp-strength-label" style="font-size:0.7rem;color:var(--text-muted);margin-top:3px;display:block;"></span>
                <span class="error-text hidden" id="cp-err-new">Password must be at least 8 characters.</span>
            </div>
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">Confirm New Password</label>
                <div style="position:relative;">
                    <input type="password" id="cp-confirm" class="form-control" placeholder="Re-enter new password" style="padding-right:42px;">
                    <button onclick="togglePwVis('cp-confirm',this)" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--text-muted);cursor:pointer;"><i class="fa-solid fa-eye"></i></button>
                </div>
                <span class="error-text hidden" id="cp-err-confirm">Passwords do not match.</span>
            </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:20px;">
            <button id="cp-save" class="btn btn-green" style="flex:1;"><i class="fa-solid fa-key"></i> Update Password</button>
            <button id="cp-cancel" class="btn btn-secondary" style="flex:1;">Cancel</button>
        </div>`;
    overlay.appendChild(dialog);
    document.body.appendChild(overlay);

    // Password strength meter
    dialog.querySelector('#cp-new').addEventListener('input', function() {
        const v = this.value;
        const fill = dialog.querySelector('#cp-strength-fill');
        const lbl = dialog.querySelector('#cp-strength-label');
        let score = 0;
        if (v.length >= 8) score++;
        if (/[A-Z]/.test(v)) score++;
        if (/[0-9]/.test(v)) score++;
        ...
        lbl.textContent = v.length ? labels[score - 1] || 'Weak' : '';
        // Real-time confirm match
        const conf = dialog.querySelector('#cp-confirm').value;
        if (conf) {
            if (v !== conf) { dialog.querySelector('#cp-confirm').classList.add('input-error'); dialog.querySelector('#cp-err-confirm').classList.remove('hidden'); }
            else { dialog.querySelector('#cp-confirm').classList.remove('input-error'); dialog.querySelector('#cp-err-confirm').classList.add('hidden'); }
        }
    });
    dialog.querySelector('#cp-confirm').addEventListener('input', function() {
        const newPw = dialog.querySelector('#cp-new').value;
        if (this.value !== newPw) { this.classList.add('input-error'); dialog.querySelector('#cp-err-confirm').classList.remove('hidden'); }
        else { this.classList.remove('input-error'); dialog.querySelector('#cp-err-confirm').classList.add('hidden'); }
    });

    const closeDialog = () => { overlay.style.opacity='0'; overlay.style.transition='opacity 0.2s'; setTimeout(()=>overlay.remove(),200); };
    dialog.querySelector('#cp-close').onclick = closeDialog;
    dialog.querySelector('#cp-cancel').onclick = closeDialog;
    overlay.addEventListener('click', e => { if (e.target === overlay) closeDialog(); });

    dialog.querySelector('#cp-save').onclick = () => {
        const current = dialog.querySelector('#cp-current').value;
        const newPw = dialog.querySelector('#cp-new').value;
        const confirm = dialog.querySelector('#cp-confirm').value;
        let ok = true;
        if (!current) { dialog.querySelector('#cp-current').classList.add('input-error'); dialog.querySelector('#cp-err-current').classList.remove('hidden'); ok = false; }
        else { dialog.querySelector('#cp-current').classList.remove('input-error'); dialog.querySelector('#cp-err-current').classList.add('hidden'); }
        if (newPw.length < 8) { dialog.querySelector('#cp-new').classList.add('input-error'); dialog.querySelector('#cp-err-new').classList.remove('hidden'); ok = false; }
        else { dialog.querySelector('#cp-new').classList.remove('input-error'); dialog.querySelector('#cp-err-new').classList.add('hidden'); }
        if (newPw !== confirm) { dialog.querySelector('#cp-confirm').classList.add('input-error'); dialog.querySelector('#cp-err-confirm').classList.remove('hidden'); ok = false; }
        if (!ok) return;
        closeDialog();
...
function togglePwVis(id, btn) {
    const el = document.getElementById(id) || btn.closest('.form-group, div').querySelector('input');
    if (!el) return;
    el.type = el.type === 'password' ? 'text' : 'password';
    btn.querySelector('i').className = el.type === 'password' ? 'fa-solid fa-eye' : 'fa-solid fa-eye-slash';
}
...
const FONT_LABELS = { small: 'Small', medium: 'Medium', large: 'Large', xlarge: 'X-Large' };

function setFontSize(size) {
    applyFontSize(size, true);
}

function applyFontSize(size, save) {
    document.documentElement.style.fontSize = FONT_SIZES[size] || '15px';
    // Update button states
    document.querySelectorAll('.font-size-btn').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.size === size);
    });
    const lbl = document.getElementById('font-size-label-display');
    if (lbl) lbl.textContent = FONT_LABELS[size] || 'Medium';
    if (save) {
...
/* =========================================================
   NEW) SETTINGS — CONTACT SUPPORT DIALOG
   ========================================================= */
function openContactSupport() {
    const support = CONFIG.supportContact;
    const overlay = document.createElement('div');
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:10000;display:flex;align-items:center;justify-content:center;';
    const dialog = document.createElement('div');
    dialog.style.cssText = 'background:var(--bg-dark-cards);border:1px solid var(--border-color);border-radius:12px;padding:28px;max-width:440px;width:90%;color:var(--text-primary);box-shadow:0 20px 60px rgba(0,0,0,0.5);';
    dialog.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <h3 style="margin:0;font-size:1.05rem;"><i class="fa-solid fa-headset" style="color:var(--accent-blue);margin-right:8px;"></i>Contact Support</h3>
            <button id="cs-close" style="background:none;border:none;color:var(--text-muted);font-size:1.2rem;cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;gap:12px;padding:12px;background:rgba(255,255,255,0.03);border-radius:8px;border:1px solid var(--border-color);">
                <i class="fa-solid fa-phone" style="color:var(--accent-green);width:18px;flex-shrink:0;"></i>
                <div><span style="font-size:0.75rem;color:var(--text-muted);">General Hotline</span><br><strong style="font-size:0.9rem;">${support.generalPhone}</strong></div>
            </div>
            <div style="display:flex;align-items:center;gap:12px;padding:12px;background:rgba(255,255,255,0.03);border-radius:8px;border:1px solid var(--border-color);">
                <i class="fa-solid fa-city" style="color:var(--accent-sky);width:18px;flex-shrink:0;"></i>
                <div><span style="font-size:0.75rem;color:var(--text-muted);">Pasay City Hotline</span><br><strong style="font-size:0.9rem;">${support.hotline}</strong></div>
            </div>
            <div style="display:flex;align-items:center;gap:12px;padding:12px;background:rgba(255,255,255,0.03);border-radius:8px;border:1px solid var(--border-color);">
                <i class="fa-regular fa-clock" style="color:var(--text-muted);width:18px;flex-shrink:0;"></i>
                <div><span style="font-size:0.75rem;color:var(--text-muted);">Office Hours</span><br><span style="font-size:0.85rem;">${support.officeHours}</span></div>
            </div>
            <div style="display:flex;align-items:center;gap:12px;padding:12px;background:rgba(255,255,255,0.03);border-radius:8px;border:1px solid var(--border-color);">
                <i class="fa-solid fa-location-dot" style="color:var(--color-red);width:18px;flex-shrink:0;"></i>
                <div><span style="font-size:0.75rem;color:var(--text-muted);">Office Address</span><br><span style="font-size:0.82rem;">${support.address}</span></div>
            </div>
        </div>
        <button id="cs-close-btn" class="btn btn-secondary btn-block">Close</button>`;
    overlay.appendChild(dialog);
    document.body.appendChild(overlay);
    const closeDialog = () => { overlay.style.opacity='0'; overlay.style.transition='opacity 0.2s'; setTimeout(()=>overlay.remove(),200); };
    dialog.querySelector('#cs-close').onclick = closeDialog;
    dialog.querySelector('#cs-close-btn').onclick = closeDialog;
    overlay.addEventListener('click', e => { if (e.target === overlay) closeDialog(); });
}

/* =========================================================
   NEW) SETTINGS — EMAIL INQUIRY DIALOG
   ========================================================= */
function openEmailInquiry() {
    ...
    dialog.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <h3 style="margin:0;font-size:1.05rem;"><i class="fa-solid fa-envelope" style="color:var(--accent-blue);margin-right:8px;"></i>Email Inquiry</h3>
            <button id="ei-close" style="background:none;border:none;color:var(--text-muted);font-size:1.2rem;cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div style="display:flex;flex-direction:column;gap:14px;">
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">Subject</label>
                <select id="ei-subject" class="form-control">
                    <option value="">Select inquiry type...</option>
                    <option>Tax Assessment Question</option>
                    <option>Payment Issue</option>
                    <option>Business Registration Help</option>
                    <option>Technical Support</option>
                    <option>Other</option>
                </select>
                <span class="error-text hidden" id="ei-err-subject">Please select a subject.</span>
            </div>
            <div class="form-group">
                <label style="font-size:0.8rem;color:var(--text-muted);">Your Message</label>
                <textarea id="ei-message" class="form-control" rows="5" maxlength="1000" placeholder="Describe your inquiry in detail..." style="resize:vertical;min-height:100px;"></textarea>
                <div style="display:flex;justify-content:space-between;margin-top:4px;">
                    <span class="error-text hidden" id="ei-err-message" style="margin:0;">Please enter a message (min 20 characters).</span>
                    <span id="ei-counter" style="font-size:0.7rem;color:var(--text-muted);margin-left:auto;">0 / 1000</span>
                </div>
            </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:20px;">
            <button id="ei-send" class="btn btn-blue" style="flex:1;"><i class="fa-solid fa-paper-plane"></i> Send Inquiry</button>
            <button id="ei-cancel" class="btn btn-secondary" style="flex:1;">Cancel</button>
        </div>`;
    overlay.appendChild(dialog);
    document.body.appendChild(overlay);

    dialog.querySelector('#ei-message').addEventListener('input', function() {
        const counter = dialog.querySelector('#ei-counter');
        counter.textContent = this.value.length + ' / 1000';
        const err = dialog.querySelector('#ei-err-message');
        if (this.value.trim().length >= 20) { this.classList.remove('input-error'); err.classList.add('hidden'); }
    });
    dialog.querySelector('#ei-subject').addEventListener('change', function() {
        const err = dialog.querySelector('#ei-err-subject');
        if (this.value) { this.classList.remove('input-error'); err.classList.add('hidden'); }
    });

    const closeDialog = () => { overlay.style.opacity='0'; overlay.style.transition='opacity 0.2s'; setTimeout(()=>overlay.remove(),200); };
    dialog.querySelector('#ei-close').onclick = closeDialog;
    dialog.querySelector('#ei-cancel').onclick = closeDialog;
    overlay.addEventListener('click', e => { if (e.target === overlay) closeDialog(); });

    dialog.querySelector('#ei-send').onclick = () => {
        const subject = dialog.querySelector('#ei-subject').value;
        const message = dialog.querySelector('#ei-message').value.trim();
        let ok = true;
        if (!subject) { dialog.querySelector('#ei-subject').classList.add('input-error'); dialog.querySelector('#ei-err-subject').classList.remove('hidden'); ok = false; }
        if (message.length < 20) { dialog.querySelector('#ei-message').classList.add('input-error'); dialog.querySelector('#ei-err-message').classList.remove('hidden'); ok = false; }
        if (!ok) return;
        const mailto = `mailto:${support.generalEmail}?subject=${encodeURIComponent('[PasayBiz] ' + subject)}&body=${encodeURIComponent(message + '\n\n---\nSent from PasayBiz Portal\nName: ' + DB.settings.name + '\nEmail: ' + DB.settings.email)}`;
        window.open(mailto, '_blank');
        closeDialog();
        toast('Email client opened with your inquiry.');
    };
}

/* Initialize currency inputs in the "Register New Business" modal (nb-gross) */
function attachNbGrossCurrencyInput(dialog) {
    const el = dialog.querySelector('#nb-gross');
    if (!el || el.dataset.currencyBound) return;
    el.dataset.currencyBound = '1';
    el.type = 'text';
    el.inputMode = 'numeric';
    el.placeholder = 'e.g., 1,000,000.00';
    attachCurrencyInput(el, null);
}